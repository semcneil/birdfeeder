...
/* rtcLoggingWifiRevisionOne.ino

   This updated version records tempature and 
   attempts to zero the load cell for more accurate readings. 
  Febuary 21 2025
*/
...
float scaleFactor = 4000; //SCALE FACTOR HAS TO BE MANUALLY SET!!
...
Adafruit_MLX90393 sensorMLX90393 = Adafruit_MLX90393();
...
void MLX90393Setup() {
  ...
  sensorMLX90393.setResolution(MLX90393_X, MLX90393_RES_17);
  sensorMLX90393.setResolution(MLX90393_Y, MLX90393_RES_17);
  sensorMLX90393.setResolution(MLX90393_Z, MLX90393_RES_16);

  // Set oversampling
  sensorMLX90393.setOversampling(MLX90393_OSR_3);

  // Set digital filtering
  sensorMLX90393.setFilter(MLX90393_FILTER_5);
  housekeepWrite("$MLX90393", "MLX90393 Setup Successful");
}

void SHT40Setup() {
  sensorSHT40.begin(Wire, 0x46);
  sensorSHT40.softReset();
    delay(10);
    uint32_t serialNumber = 0;
    error = sensorSHT40.serialNumber(serialNumber);
    if (error != NO_ERROR) {
        housekeepWrite("$ERROR","Error trying to execute serialNumber() for SHT40: ");
    }
}
...
void tempPressure(int interval = 0) {
  ...
  error = sensorSHT40.measureLowestPrecision(aTemperature, aHumidity);
  if (error != NO_ERROR) {
    Serial.print("Error trying to execute measureLowestPrecision(): ");
    errorToString(error, errorMessage, sizeof errorMessage);
    Serial.println(errorMessage);
    housekeepWrite("$ERROR",errorMessage);
  }
  outStr = "SHT40 Temperature: ";
  outStr = outStr + String(aTemperature*1.8+32);
  outStr = outStr + "Fahrenheit\t";
  outStr = outStr +"SHT40 Humidity: ";
  outStr = outStr + String(aHumidity);
  housekeepWrite("$SHT40",outStr);
  Serial.println(outStr);
  ...
  if ((curMillis - lastWrite) >= interval) {
    ...
    outStr = "BMP390 Temperature = ";
    outStr = outStr + String(bmp.temperature);
    outStr = outStr + " *C\t";
  
    outStr = outStr + "BMP390 Pressure = ";
    outStr = outStr + String(bmp.pressure);
    outStr = outStr + " Pa";
    Serial.println(outStr);
    housekeepWrite("BMP390",outStr);
...
void magnetometer(int interval = 0) {
  ...
  if ((curMillis - lastWrite) >= interval) {
    ...
    if (sensorMLX90393.readData(&x, &y, &z)) {
...
int zeroLoadCell(int interval) {
  unsigned long curMillis = millis();
  static unsigned long lastMillis = curMillis;
  if (interval <= (curMillis - lastMillis)) {
    lastMillis = curMillis;
    int zerod = nau.read(); // Capture baseline value
    Serial.print("Rezoring... Zero Offset: ");
...
void loadRFIDWrite() {
  ...
  if(nau.available()) {
    ...
    val = ((val - zeroOffset) / scaleFactor);
    if((curMillis - lastTagReadTime) < afterReadRecordTime) {
      ...
      if (myFileLoadCell) {
        ...
        Serial.print(" \t\t ");
        // close the file:
        myFileLoadCell.println(curTimeStr(rtc.now()) + "," + String(curMillis) + "," + String(val) + "," + String(zeroOffset) + "," + bmp.temperature + "," + bmp.pressure);
...
void timeLog(int interval) {
  ...
  if (interval <= (curMillis - lastMillis)) {
    ...
    if (myFileLoadCell) {
      Serial.print(" \t\t ");
      // close the file:
      myFileLoadCell.println(curTimeStr(rtc.now()) + "," + String(millis()) + "," + "" + "," + "" + "," + bmp.temperature + "," + bmp.pressure);
      myFileLoadCell.close();
    } else {
      // if the file didn't open, print an error:
      Serial.println("error opening " + fname);
...
void setup(void) {
  ...
  delay(2000); // Wait for user to remove weight
  zeroOffset = nau.read(); // Capture baseline value
  ...
  if(myFileLoadCell) {
    myFileLoadCell.println("datetime,time(ms),LoadCell,Zero Offset, Tempature, Pressure, RFIDTag");
    myFileLoadCell.close();
  } else {
...
void loop(void) {
  ...
  zeroOffset = zeroLoadCell(60000); //Run zeroLoadCell() every minute
  
  timeLog(60000); //Run timeLog() every minute
