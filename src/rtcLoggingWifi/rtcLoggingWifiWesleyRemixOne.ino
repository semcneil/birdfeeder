...
int timeZone = 0;                       //Timezone. Default is UTC-0
const int loopsBetweenReads = 100;       //If a tag is read and it is the same as the last tag read, wait this many loops before logging it again. Default is 10.
const int loopsBetweenRezero = 1000;     //How many loops between rezeoring of the load cell. Default is 1000
const int loopsBetweenTimeLogs = 10000;  //How many loops between logging the time and tempature. Default is 10000;
const float scaleFactor = 4000;         //Load Cell Readings are divided by this to get bird weight. Default is 4000
...
int zeroLoadCell() {
  ...
  while (retry < 5) {  // Retry up to 5 times
    zerod = nau.read();
    if (zerod != 0) break;  // Exit if valid reading
  ...
  Serial.print("Zeroing... Zero Offset: ");
...
void checkRFID() {
  if (Serial1.available() > 0) {  //Check if RFID is Reading Anything
    ...
    while (Serial1.available() > 0 && counter < 4) {
      byte newTag = Serial1.read();
      tagNum += String(newTag, HEX);
      counter++;
      delay(1);
    }
    Serial.println("Tag detected: " + String(tagNum));
    if (nau.available()) {                                //Check if Load Cell is Available
      if (loopSinceLastRead > loopsBetweenReads || prevTag != tagNum) {  //
        myFileLoadCell = SD.open(fname, O_WRITE | O_APPEND);
        if (myFileLoadCell) {
          loopSinceLastRead = 0;
          double val = nau.read();
          val = ((val - zeroOffset) / scaleFactor);
          myFileLoadCell.println(curTimeStr(rtc.now()) + "," + String(millis()) + "," + String(val) + "," + String(zeroOffset) + "," + String(bmp.temperature) + "," + String(bmp.pressure) + "," + String(tagNum));
          myFileLoadCell.close();
          prevTag = tagNum;
          tagNum = "";
          Serial.println("Tag Written to File");
        } else {
          Serial.println("Error Inside checkRFID: Failed To Open File");
        }
      }
    } else {
      Serial.println("Error Inside checkRFID: Load Cell is unavailable");
    }
  }
...
void timeLog() {
  ...
  if (myFileLoadCell) {
    myFileLoadCell.println(curTimeStr(rtc.now()) + "," + String(millis()) + "," + " " + "," + String(zeroOffset) + "," + String(bmp.temperature) + "," + String(bmp.pressure) + "," + "");
    myFileLoadCell.close();
    Serial.println("Time Log Written to File");
  } else {
    Serial.println("Error inside timeLog: Failed to Open File");
  }
}
...
void setup(void) {
  ...
  zeroOffset = zeroLoadCell();  // Capture baseline value
...
void loop(void) {
  ...
  if (loopCount % loopsBetweenRezero == 0) {  //Every Ten Thousand Loops Zero again

    zeroOffset = zeroLoadCell();
    delay(100);
  }
  if (loopCount % loopsBetweenTimeLogs == 0) {  //Every One Hundred Thousand Loops Record Time and Temp
    timeLog();
    delay(100);
  }
  checkRFID();
